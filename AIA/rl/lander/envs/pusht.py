    ...
    * TODO:
    ...
    def step(self, action):
        ...
        for _ in range(n_steps):
            # 1) compute PD‐control acceleration
            acceleration = self.k_p * (action - self.agent.position) \
                           + self.k_v * (Vec2d(0, 0) - self.agent.velocity)

            # 2) update velocity
            new_vel = self.agent.velocity + acceleration * self.dt

            # 3) clamp each component into [-30, +30]
            vx = np.clip(new_vel.x, -30.0, 30.0)
            vy = np.clip(new_vel.y, -30.0, 30.0)
            self.agent.velocity = Vec2d(vx, vy)

            # 4) update position
            self.agent.position += self.agent.velocity * self.dt

            # 5) step the rest of the physics
